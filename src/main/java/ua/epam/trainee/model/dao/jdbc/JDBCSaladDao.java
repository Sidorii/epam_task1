...
import ua.epam.trainee.model.dao.jdbc.mappers.ExtractType;
import ua.epam.trainee.model.dao.jdbc.mappers.ObjectMapper;
import ua.epam.trainee.model.dao.jdbc.mappers.SaladMapper;
import ua.epam.trainee.model.dao.jdbc.transactions.TransactionalConnection;
...
public class JDBCSaladDao extends JdbcCrudDao<Salad> implements SaladDao {
    ...
    public Salad getSaladByName(String name) {
        try (Connection connection = getConnection()) {
            return getByName(name, connection);
        } catch (SQLException e) {
            e.printStackTrace();
            throw new MissingEntityException("Salad by name: " + name + " not found");
        }
    }
    ...
    private Salad getByName(String name, Connection connection) throws SQLException {
        final String query = "" +
                " SELECT * " +
                " FROM task1.salad " +
                " LEFT JOIN task1.salad_ingredient " +
    ...
    @Override
    public Salad find(Salad entity, Connection connection) throws MissingEntityException, SQLException {
